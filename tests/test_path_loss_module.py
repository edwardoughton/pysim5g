...
#     iterations, expected):
#     # print(free_space(frequency, distance, ant_height, ue_height, seed_value,
#     #     iterations))
#     assert (
...
def test_extended_hata_model_value_errors():
    ...
    with pytest.raises(ValueError, match='Carrier frequency incorrect for Extended Hata'):
        extended_hata(7,200,20,'macro',20,20,'urban','los',1.5,1,42,1)

    with pytest.raises(ValueError, match='Could not determine if cell is above or below roof line'):
...
@pytest.mark.parametrize("frequency, distance, ant_height, ant_type, \
    building_height, street_width, settlement_type, type_of_sight, \
    ue_height, seed_value, iterations, expected", [
    (3.5,500,10,'micro',20,20,'urban','los',1.5,42,1,round(98.26+2.86,2)),
    (3.5,1000,10,'micro',20,20,'urban','los',1.5,42,1,round(107.72+2.86,2)),
    ## (3.5,6000,10,'micro',20,20,'urban','nlos',1.5,42,1,round(175+3.02)),
    (3.5,500,10,'micro',20,20,'urban','nlos',1.5,42,1,round(135.90+3.02,2)),
    (3.5,500,25, 'macro',20,20,'urban','los',1.5,42,1,round(98.26+3.02,2)),
    (3.5,2000,25,'macro',20,20,'urban','los',1.5,42,1,round(112.6+3.02,2)),
    ## (3.5,6000,10,'macro',20,20,'urban','los',1.5,42,1, round(136.62+2.74)),
    (3.5,1000,25,'macro',20,20,'urban','nlos',1.5,42,1,round(141.69+3.19,2)),
    ## (3.5,6000,10,'macro',20,20,'urban','nlos',1.5,42, round(191.91),
    (3.5,1000,35,'macro',10,20,'suburban','los',1.5,42,1,round(107.74+3.02,2)),
    (3.5,4000,35,'macro',10,20,'suburban','los',1.5,42,1,round(126.73+3.02+3.19,2)),
    (3.5,500,35,'macro',10,20,'suburban','nlos',1.5,42,1,round(121.39+3.26,2)),
    ## (3.5,6000,10,'macro',80,20,'suburban','los',1.5,42, 164.12),
    (3.5,1000,35,'macro',10,20,'rural','los',1.5,42,1,round(107.74+3.02,2)),
    (3.5,4000,35,'macro',10,20,'rural','los',1.5,42,1,round(126.73+3.02+3.19,2)),
    (3.5,500,35,'macro',10,20,'rural','nlos',1.5,42,1,(121.39+3.26)),
    # (3.5,6000,10,'macro',80,20,'rural','nlos',1.5,42,1,418.36),
    ])
...
@pytest.mark.parametrize("frequency, distance, ant_height, ant_type, \
    ...
    (3.5,500,35,'macro',10,20,'suburban','nlos',1.5,0,False,42,1,(round(121.39+3.26,2), 'e_utra_3gpp_tr36_814')),
    # (1.8,500,40,'',20,20,'urban','nlos',1.5,0,True,42,1,(round(129.12+0.16,2), 'extended_hata_path_loss')),
    (3.5,500,40,'macro',10,20,'suburban','nlos',1.5,0,True,42,1,(round(119.94+3.26+5.05,2), 'e_utra_3gpp_tr36_814')),
])
